...
#define MAX_PRINT_BYTES    (4096) /* print max of 4096 bytes per col/chunk value */
#define BUF_SIZE           80
...
#define PRINT_POSITION(print_txt, pos, pos_len) \
do { \
  ub2  _idx; \
  printf("%s", (print_txt)); \
  for (_idx = 0; _idx < (pos_len); _idx++) \
    printf("%02x", (pos)[_idx]); \
  printf("\n"); \
} while (0)
...
static void get_lcrs(oci_t * ocip)
{
  ...
  ub1         lcrtype;
  oraub8      flag;
  ub1         fetch_pos[OCI_LCR_MAX_POSITION_LEN];
  ub2         fetch_pos_len = 0;
  ...
  while (status == OCI_SUCCESS)
  {
    ...
    while ((status = OCIXStreamOutLCRReceive(ocip->svcp, ocip->errp,
                                   &lcr, &lcrtype, &flag, 
                                   fetch_pos, &fetch_pos_len, OCI_DEFAULT))
                          == OCI_STILL_EXECUTING)
    {
...
/*    if (fetch_pos_len)
      PRINT_POSITION("Fetch position", fetch_pos, fetch_pos_len);
*/
  }
}
...
static void print_lcr(oci_t *ocip, void *lcrp, ub1 lcrtype)
{
  ...
  ret = OCILCRHeaderGet(ocip->svcp, ocip->errp, 
                        ...
                        &lcr_position, &lcr_position_len,    /* LCR position */
                        &lcr_flags, lcrp, OCI_DEFAULT);
  ...
  else
  {
    ...
    PRINT_POSITION("Position=", lcr_position, lcr_position_len);

    if (lcr_flags & OCI_ROWLCR_HAS_ID_KEY_ONLY) 
      printf("LCR has ID key only.\n");
  ...
  if (IS_COMMIT_CMD(cmd_type, cmd_type_len))
  {
    PRINT_POSITION("\nSetting processed LWM to ", 
                   lcr_position, lcr_position_len);
    ret = OCIXStreamOutProcessedLWMSet(ocip->svcp, ocip->errp,
                    lcr_position, lcr_position_len, OCI_DEFAULT); 
  }
}
